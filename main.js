...
bot.on("callback_query", (callbackQuery) => { //function to respond to received button
    ...
    //console.log(msg.chat.id)
		if (callbackQuery.data==="canada"){
        	 ...
        	 let phrase2=text.slice(15)
			        ...
			        bot.answerCallbackQuery(callbackQuery.id)
			            .then(() => bot.sendMessage(msg.chat.id, "Weather in " + phrase1 + " is " + phrase2)).then(reset(msg));
    	...
    	bot.sendDocument(msg.chat.id,"/karmaniy_spravochnik_po_piton.zip").then(reset(msg))
				...
				if ( stop_listening === false){
							(async () => {
							 ...
							 for(let i = 0; i<all_users.length; i++){
								...
								bot.sendMessage(msg.chat.id, "Успешно отправлено!")
							}
							)();
							reset(msg)
